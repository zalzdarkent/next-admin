...
import ContentDelay from '@/components/ui/content-delay'
...
export default function PagesLayout({ children }: PagesLayoutProps) {
  ...
  // Generate breadcrumb from pathname for pages
  const generateBreadcrumb = () => {
    ...
    paths.forEach((path, index) => {
      currentPath += `/${path}`
      if (index > 0) { // Skip 'pages' as we want to map it to relevant names
        let name = path.charAt(0).toUpperCase() + path.slice(1)
        
        // Map specific page names for better UX
        if (path === 'users') name = 'Users'
        if (path === 'reports') name = 'Reports'
        if (path === 'account') name = 'My Account'
      ...
      <div className={`transition-all duration-500 ease-in-out ${isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64'}`}>
        ...
        <div className="p-4 md:p-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 min-h-[calc(100vh-2rem)] md:min-h-[calc(100vh-3rem)]">
            ...
            {/* Main content - Hanya area ini yang akan berubah */}
            <div className="p-4 md:p-6">
              <ContentDelay>
                {children}
              </ContentDelay>
